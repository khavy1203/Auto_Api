...
const getInfoStudentForLocal = async (tokenLocalNLTB = null, CCCD) => {
    try {
        console.log("CCCD đã xác nhận chuẩn bị truy vấn:", CCCD)
        const yourBearToken = tokenLocalNLTB ? tokenLocalNLTB : process.env.tokenLocalNLTB;
        ...
        return await getSessionStu.get(`/api/HocVienTH?soCmt=${CCCD.trim()}&page=1&limit=10`)
            .then(response => {
                console.log("Check url", `/api/HocVienTH?soCmt=${CCCD}&page=1&limit=10`)
                console.log("check response", response.status)
